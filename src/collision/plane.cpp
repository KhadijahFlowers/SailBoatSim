...
//  nanogui::Color color(0.1f, 0.7f, 0.7f, 1.0f);
//
...
Plane::Plane(double width, double height, int num_width_points,
...
double Plane::distance3D(double x1, double y1, double z1, double x2, double y2, double z2) {
	return (double)sqrt(pow((x1 - x2), 2) + pow((y1 - y2), 2) + pow((z1 - z2), 2));
}



void Plane::simulate(double frames_per_sec, double simulation_steps, PlaneParameters *cp,
...
void Plane::self_collide(PointMass &pm, double simulation_steps) {
...
float Plane::hash_position(Vector3D pos) {
